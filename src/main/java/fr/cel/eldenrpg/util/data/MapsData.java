...
public final class MapsData {
    ...
    public static boolean addMapId(IPlayerDataSaver player, int mapId) {
        int[] mapsId = player.getPersistentData().getIntArray("mapsId").get();
        ...
        player.getPersistentData().putIntArray("mapsId", newMapsId);
        syncMap((PlayerEntity) player, mapId);
        return true;
    }

    public static int[] getMapsId(IPlayerDataSaver player) {
        return player.getPersistentData().getIntArray("mapsId").get();
    }
